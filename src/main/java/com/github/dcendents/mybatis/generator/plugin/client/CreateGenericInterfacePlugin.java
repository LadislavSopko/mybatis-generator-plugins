...
public class CreateGenericInterfacePlugin extends PluginAdapter {
	...
	void addGenericMethod(Method method, FullyQualifiedJavaType returnType, FullyQualifiedJavaType... types) {
		...
		if (!methodsAdded.contains(method.getName())) {
			...
			// add RowBounds versions (those are added by other plugin so non visible
			if(method.getName() != null && (method.getName().equals("selectByExample") || method.getName().equals("selectDistinctByExample"))) {
				
				// selectByExample(RowBounds rowBounds)
				Method gm = new Method(method.getName());
				setSomeDataOnMethod(gm, returnType, genericMethod, types);
				gm.addParameter(new Parameter(new FullyQualifiedJavaType("org.apache.ibatis.session.RowBounds"), "rowBounds"));
				genericInterface.addMethod(gm);
				
				// selectByExample(BasicColumn ... selectList)
				Method gm1 = new Method(method.getName());
				setSomeDataOnMethod(gm1, returnType, genericMethod, types);
				gm1.addParameter(new Parameter(new FullyQualifiedJavaType("org.mybatis.dynamic.sql.BasicColumn"), "selectList", true));
				genericInterface.addMethod(gm1);

				// selectByExample(RowBounds rowBounds, BasicColumn ... selectList)
				Method gm2 = new Method(method.getName());
				setSomeDataOnMethod(gm2, returnType, genericMethod, types);
				gm2.addParameter(new Parameter(new FullyQualifiedJavaType("org.apache.ibatis.session.RowBounds"), "rowBounds"));
				gm2.addParameter(new Parameter(new FullyQualifiedJavaType("org.mybatis.dynamic.sql.BasicColumn"), "selectList", true));
				genericInterface.addMethod(gm2);
	 ...
	 * @param types				disponible types
	 */
	private void setSomeDataOnMethod(Method gm, FullyQualifiedJavaType returnType, Method method, FullyQualifiedJavaType... types) {
		if (!this.suppressAllComments) {
			gm.addJavaDocLine("/**");
			gm.addJavaDocLine(" * This method was generated by MyBatis Generator.");
			gm.addJavaDocLine(" *");
			gm.addJavaDocLine(" * @mbg.generated");
			gm.addJavaDocLine(" */");
		}
		gm.addAnnotation("@Generated(\"org.mybatis.generator.api.MyBatisGenerator\")");
		gm.setReturnType(returnType);
		gm.setDefault(true);
		...
		for (int i = 0; i < method.getParameters().size(); i++) {
			...
			Parameter genericParameter = new Parameter(paramType, parameter.getName());
			gm.addParameter(genericParameter);
		}
		
		gm.getBodyLines().clear();
		String castType = returnType.isPrimitive() ? returnType.getPrimitiveTypeWrapper().getShortName()
				: returnType.getShortName();
		gm.addBodyLine(String.format("return (%s)null;", castType));
	}
