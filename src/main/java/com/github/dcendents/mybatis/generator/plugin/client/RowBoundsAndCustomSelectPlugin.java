...
public class RowBoundsAndCustomSelectPlugin extends PluginAdapter {
    ...
    private void addNewComposedFunction(Interface interfaze, IntrospectedTable introspectedTable,
            FullyQualifiedJavaType baseMethodReturnType) {
    ...
    private void copyAndAddSelectByExampleMethodForDSQL(Method method, Interface interfaze) {
        ...
        Method newMethod1 = new Method(method);
        newMethod1.addParameter(new Parameter(basicColumn, "selectList", true)); //$NON-NLS-1$
        interfaze.addMethod(newMethod1);
        
        // replace the call to selectMany with the new call to selectManyWithRowbounds
        for (int i = 0; i < newMethod1.getBodyLines().size(); i++) {
            String bodyLine = newMethod1.getBodyLines().get(i);
            
            if (bodyLine.contains("this::selectMany")) { //$NON-NLS-1$
            	
            	if(bodyLine.contains("selectDistinctWithMapper")) {
            		bodyLine = "return SelectDSL.selectDistinctWithMapper(this::selectMany, selectList)";
            	} else {
            		bodyLine = "return SelectDSL.selectWithMapper(this::selectMany, selectList)";
            	}
                
                newMethod1.getBodyLines().set(i, bodyLine);
                break;
            }
        }
        
        /* selectByExample(BasicColumn...selectList) */
        Method newMethod2 = new Method(method);
        newMethod2.addParameter(new Parameter(rowBounds, "rowBounds")); 
        newMethod2.addParameter(new Parameter(basicColumn, "selectList", true)); //$NON-NLS-1$
        interfaze.addMethod(newMethod2);
        
        // replace the call to selectMany with the new call to selectManyWithRowbounds
        for (int i = 0; i < newMethod2.getBodyLines().size(); i++) {
            String bodyLine = newMethod2.getBodyLines().get(i);
            
            if (bodyLine.contains("this::selectMany")) { //$NON-NLS-1$
            	
            	if(bodyLine.contains("selectDistinctWithMapper")) {
            		bodyLine = "return SelectDSL.selectDistinctWithMapper(selectManyWithRowbounds(rowBounds), selectList)";
            	} else {
            		bodyLine = "return SelectDSL.selectWithMapper(selectManyWithRowbounds(rowBounds), selectList)";
            	}
                
                newMethod2.getBodyLines().set(i, bodyLine);
                break;
            }
        }
    }
